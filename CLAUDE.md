...
| `scripts/generate_ai.py` | Gemini AI analysis from latest deltas; writes `data/ai/YYYY-MM-DD.json`. Model `gemini-3.8-flash` at `THINKING_LEVEL="low"`. Auth: Vertex express key (GOOGLE_API_KEY) > Vertex ADC > AI Studio (GEMINI_API_KEY). Supports `--preview` (no API), `--capture` (Tier-2 debug). **Spend controls (AI-WALLET):** `AI_DISABLED=1` kill switch, `MAX_API_CALLS_PER_RUN` runaway guard, input-diff dedupe of redundant runs, and per-run actual-cost logging → `data/ai/spend.json`. See `scripts/CLAUDE.md` § AI spend controls. | ~1300 |
| `scripts/ai_cost.py` | Reusable cost meter: date-aware Gemini pricing (`price_for`, handles the 2027-01-01 intro-price cliff) + `cost_of_usage`. Shared by `generate_ai.py` now and the AI-NEXT workstream later. | ~60 |
...
- **Sync first — HARD RULE, not just at session start**: Run `git fetch origin && git log --oneline origin/claude/elegant-babbage-hlxnfy -5` **before any code investigation and before starting any new change** — not only at session start. Re-fetch **again before each new work phase within the same session**: a PR you opened earlier can be merged by the human *while you keep working*, which strands any commit you push to that already-merged branch (it sits on the feature branch with no path into default). If you are about to branch, edit, or push and you have not fetched since the last merge/hand-off, fetch first. Before pushing follow-up work, confirm the target PR is still open (`git branch -r --contains <your-last-commit>` should NOT show it already in default). When a PR is already merged, per `.claude/rules/branch-commit-discipline.md` § Amendment policy: restart the branch from latest default (keeping any unmerged commits via rebase/cherry-pick) and open a **new** PR — never push onto merged history.
- **Tracking is MANDATORY inside the PR — never ask, never defer**: Every PR that ships or defers work MUST carry its own session-notes update, its `.session/SPRINT.md` task-state update, and a tracked item (GitHub issue or SPRINT task ID) for **every** deferred/follow-up idea mentioned anywhere in the PR or conversation. Do not ask the user whether to add session notes / SPRINT / tracking — it is required; just do it in the same PR (or an immediate follow-up PR that is merged before session end, per the session-notes-must-land-on-default rule). A follow-up idea that lives only in a PR description or chat is considered lost — open an issue or SPRINT task for it the moment it is raised, so nothing gets orphaned.
