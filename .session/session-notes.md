...
**Status (2026-06-22) — AI CAPTURE & VISIBILITY (planning): APPROVED, IMPL ON HOLD.** Branch `claude/vibrant-knuth-xrj9d9`, PR #154. Brainstorm + plan only — no code yet. Deliverables: `planning/ai-capture-and-visibility.md` (revised per code-review bot) + `knowledge/decisions/ADR-006-ai-call-capture.md`. Design: two-tier capture from one hook in `generate_for_group()` — Tier-1 provenance (input block, committed permanently → PWA "Behind this" drawer) and Tier-2 debug (full prompt+raw+parsed+usage+latency, **committed with rolling 30d in HEAD, NOT gitignored** — VP corrected the original gitignore-Tier-2 draft since CI runners would lose it). VP decisions locked: `_call_api()` → `CallResult` dataclass; merge PR #154; **hold implementation**. Review blockers resolved: prompt determinism audited (only `date_str` is non-CSV — fixtures pin it); `collect.yml:174` `git add data/` already stages provenance; `google-genai>=2.8.0` exposes `usage_metadata`; `GOOGLE_API_KEY` (Vertex express key) confirmed present in this env. Execution order when greenlit: 1(capture+CallResult) → 2(`--preview`) → 7(express-key auth) → 3(snapshot tests) → 4(provenance+PWA drawer) → 5(`tools/ai-lab.html`) → 6(`eval_ai.py`). Remaining open decisions in plan: provenance granularity, CI-artifact archive, eval cadence. **Next session: await VP go-ahead, then start Phase 1.** CI repo secret for `GOOGLE_API_KEY` still TODO for nightly path.
