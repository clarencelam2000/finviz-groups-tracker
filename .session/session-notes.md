...
**Status (2026-06-19) — CLOUDFLARE CRON SCHEDULER:** Implemented Phases 1, 2, 4 of `planning/cloudflare-cron-scheduler.md` on branch `claude/wizardly-edison-l2642g`. NEW `worker-cron/` Worker (`finviz-cron-dispatcher`): `scheduled()` POSTs GitHub `workflow_dispatch` for `collect.yml` on the three weekday crons (49 13 / 51 14 / 48 19 UTC), records outcome to `DISPATCH_LOG` KV; `GET /health` + `/last` debug endpoints. 13 Vitest tests green (offline). Added `worker-cron-test` job to `.github/workflows/tests.yml`. Trimmed `collect.yml` to a single same-time EOD backstop cron (`48 19 * * 1-5`), `workflow_dispatch` now primary. Docs: CLAUDE.md § Automation rewritten, README § Configurable parameters new "Scrape schedule" table, ADR-004. Python suite 164 pass. **Phase 3 (edge-scrape Browser Rendering spike) DEFERRED to its own session** — it's live research, doesn't fit this PR. **VP action items before deploy:** (1) mint GitHub fine-grained PAT (this repo, Actions: R/W) → `GITHUB_DISPATCH_TOKEN` set via `wrangler secret put`; (2) `wrangler kv namespace create DISPATCH_LOG` and paste id into `worker-cron/wrangler.toml` (currently `REPLACE_WITH_KV_NAMESPACE_ID`). The `GITHUB_DISPATCH_TOKEN` env var exists in *this Claude env* but the Worker needs it as a `wrangler secret`. **Safe to close once PR opened/merged.**
