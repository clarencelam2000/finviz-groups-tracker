...
**Status (2026-06-21) — VS MARKET TAB FIXES: COMPLETE on `claude/beautiful-hamilton-l2qk3m`.** Draft PR opened. 282 non-Playwright tests pass (9 pre-existing Playwright/Streamlit env failures, unchanged from base). Reported by user from PWA screenshots: NH badge on 100% of cards, misleading "freshly beating the market" regime labels, unlabeled regime number, header/tab overlap. Fixes (3 commits): (1) `df35697` gate `rs_new_high` behind `RS_NEW_HIGH_MIN_SESSIONS` (=20) so it stays NaN until enough history — root cause was a 20-session high computed over only 8 sessions flagging everything; +regression test, CLAUDE.md/README. (2) `6d693b0` surgically blank stale `rs_new_high` on 2026-06-18 (only date affected; other columns byte-identical). (3) `56dceb0` PWA: regime headers now "short-term RS improving/weakening vs long-term", labelled regime number, NH/cross hover titles, header subtitle `truncate`+`min-w-0` to stop tab-bar overlap; release surface (releases.json 2026.06.21.3, sw.js v22). Sonnet subagent review run next. **Don't close yet — PR review in progress.**
