...
| 2026-06-14 | AI-MIGRATION: Gemini AI Studio → Vertex AI (Phases 2–4, PR #80 merged) | Dual-mode client in `generate_ai.py` (toggle `GOOGLE_GENAI_USE_VERTEXAI`), WIF auth in `generate_ai.yml`, 6 new tests (169 passing). Removes the 20 RPD free-tier wall; routes spend through $10/mo Vertex-only credits. Backward-compatible. Phase 1 GCP setup (3 repo secrets) pending owner action before AI runs activate. Plan: `planning/vertex-ai-migration.md`. |
