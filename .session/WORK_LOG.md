...
| 2026-06-14 | AI-MIGRATION: Gemini AI Studio → Vertex AI (Phases 2–4, code complete) | Dual-mode client in `generate_ai.py` (toggle `GOOGLE_GENAI_USE_VERTEXAI`), WIF auth in `generate_ai.yml`, 6 new tests (169 passing). Removes the 20 RPD free-tier wall; routes spend through $10/mo Vertex-only credits. Backward-compatible. **Gated on owner Phase 1 GCP setup before merge.** Plan: `planning/vertex-ai-migration.md`. |
