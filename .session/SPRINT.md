...
| PICKS-STATE-PERSIST | **[FAST-FOLLOW] Retain Picks tab expanded-row + All/Focus view state across tab navigation** | `docs/index.html` | M | Scroll position retention shipped (PICKS-CHARTS above). Deferred: (1) expanded risk-panel rows collapse on every re-entry because `renderPicks()` does a full `innerHTML` rebuild — needs a persisted identity key per row (ticker + list_category; the current DOM key also bakes in a `rowIndex` that isn't stable across renders) plus a change to `renderPickRow`/`renderLookupStage2` to start a row expanded when its key is in the persisted set. (2) the All/Focus view selection always resets to All on tab entry — this is the existing intentional **A4** behavior (`switchTab` forces `picksView = 'all'`, see PICKS-3B); retaining it would reverse that prior decision and needs an explicit call, not a silent change. Discussed with owner 2026-06-30 — do in a follow-up PR. |
