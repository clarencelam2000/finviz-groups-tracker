...
| ~~INS-7~~ | ~~**Sector Breadth metric**~~ | `dashboard/app.py`, `docs/index.html` | M | ✅ **Streamlit done 2026-06-24 (PR #177).** PWA side landed as a *different* feature than originally scoped here — see HIR-A/HIR-B below: a breadth table shipped to the PWA Strength tab (`122a4d1`, merged), while the Today-tab sector-card breadth bar is built but unmerged (PR #178, see HIR-B). |
...
| # | Task | File(s) | Effort | Notes |
|---|------|---------|--------|-------|
| HIR-A | **PWA drill-down navigation** | `docs/index.html` | M | ⚠️ **Already built, unmerged.** Draft PR #178 (`claude/fervent-thompson-rlvfs1`) implements this per the 2026-06-24 VP decision (expand-in-place). 177 commits stale as of 2026-07-02; real but reconcilable conflict in `docs/index.html` against the Strength-tab breadth table that shipped separately (`122a4d1`). **Do not re-implement — rebase PR #178 and reconcile the two taxonomy-loading code paths.** ~1 session. |
| HIR-B | **PWA breadth bar on sector cards (Today tab)** | `docs/index.html` | S | ⚠️ **Already built, unmerged** — same PR #178 as HIR-A (count + mini-bar, per VP decision). Distinct from the Strength-tab breadth *table* that already shipped (`122a4d1`) — both are wanted, they serve different tabs. |
| HIR-F | **Rank within sector** | `docs/index.html` | S | ⚠️ **Already built, unmerged** — same PR #178. Shown in the drill-down only (VP decision: not on the collapsed card). |
| HIR-R | **Market-wide breadth gauge** | `docs/index.html` | S | "68% of industries top-half" at top of PWA home. Risk-on/risk-off dial. Not started. |
