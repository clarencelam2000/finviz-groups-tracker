...
| WS5-3b | **`advance()` wiring: D1 persistence + trigger** | `worker-positions/**`, feed/cron | M | 🔴 Next. Load a position + trailing `ticker_quotes` bars → `advance()` → persist new spine state + append `position_events` → `last_advanced_date` DB idempotency; service-token `/advance` route (or ingest-triggered sweep) + daily trigger after the held ingest. Gated on a few accumulated bars for a live dry-run. |
