...
| PICKS-4 | **Phase 4a: group-level alpha scoreboard (`evaluate_picks.py`)** | `scripts/evaluate_picks.py` (new), `tests/test_evaluate_picks.py` (new), `data/picks/eval/group_scores.csv` | M | **Full spec: `planning/picks-alpha-evaluation.md` Part 1.** Daily batch (no scrape) run after `compute_deltas.py`: appends forward 1/3/5/10-session group returns vs SPY **and** vs the cross-sectional industry median (the honest alpha control), tagged by `list_category` bucket, last-write-wins per `(pick_date, group, horizon)` with an `n_sessions_avail` settle column. `--report` prints the roll-up + the paired per-date test + a sample-size guard. **This is the highest-leverage Picks-alpha item** — makes every future re-assessment a `--report` call instead of a hand analysis. Baseline it automates: `knowledge/investigations/picks-alpha-assessment-2026-07-14.md`. Wire into CI as a **separate** follow-up (3rd `data/` writer — must join `finviz-data-commit` concurrency + rebase, see AUD-4). |
