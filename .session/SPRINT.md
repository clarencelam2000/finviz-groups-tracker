...
| WS5-8b | **Personal watchlist that rides the Morning scrape** (#319) | `worker-positions/**`, `scripts/{pick_status,collect_morning}.py`, `docs/**`, `tests/**` | L | 🟡 **DESIGN LOCKED — ready to build. Cold-start brief: `planning/watchlist-build-brief-8b.md`** (authoritative; supersedes §8b's think-big). Owner design session 2026-08-15 (PR #321, mocks `planning/mocks/ws5-watchlist-{directions,surface}.html`). Key reframe: NOT public-picks force-include — private D1 `watchlist` table (membership+level+TTL), union watch tickers into the **morning + held** feeds, ONE status engine (`pick_status.py`, no drift). Watch item ≠ ticket (no stop/size); §8a form reused only at **graduation**. Locked: privacy posture (a) anonymous-public quote; N=10 mornings; your-level = direction+quiet-met, no "crossed"; new `reclaim` engine state (today OR yesterday low); gauge on-by-default; level types above/below/20MA/50MA. Phasing **P1 worker/D1 ✅ (0003_watchlist.sql + CRUD + /watchlist-tickers + /watchlist/tick + heldTickers union + src/watchlist.js + 36 vitest, 209 total green)** → P2 feed+engine (`pick_status.py` reclaim + `collect_morning.py` union) → P3 PWA. Deferred (tracked in brief §10): fully-private morning store, multi-day reclaim, picks opting into reclaim. **P1 deviation flagged:** `/watchlist-tickers` omits `level_value` (privacy; CI path never needs it — `GET /watchlist` still returns it). |
