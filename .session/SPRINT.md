...
| WS5-8a | **Manual "any ticker" position entry (§8a, #264)** | `docs/index.html`, `docs/sw.js`, `docs/releases.json`, `docs/CLAUDE.md`, `worker-positions/src/positions.js`, `worker-positions/test/positions.test.js`, `tests/test_pwa_*.py`, `.github/workflows/tests.yml` | M | 🟡 **In progress (this session, branch `claude/any-ticker-entry-form-guwbl2`).** PWA form to open a position on any typed ticker (Positions tab, collapsed expander) — Option B "guided ticket" per the *Manual Position Entry* design review. Payload identical to picks path + `meta.source='manual'`, `stop_basis='manual'`. Refinements: bidirectional sizing (size by risk $ ↔ by shares), stop as price ↔ %-below-entry, optional `days_to_earnings`, optional **backdated `entry_date`** (small worker add: validator accepts it, `buildPositionRow` uses it else stamps today; engine still advances forward). Lookup tab gets an "Open a position →" redirect that deep-links + prefills. `posCardHtml` already renders `source`/`manual` — zero list changes. |
