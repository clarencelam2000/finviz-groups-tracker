...
| # | Task | File(s) | Effort | Notes |
|---|------|---------|--------|-------|
| AI-MIGRATION | **Migrate from Gemini AI Studio to Vertex AI (Phases 1–4)** | `scripts/generate_ai.py`, `.github/workflows/generate_ai.yml`, `tests/test_generate_ai.py`, `CLAUDE.md` | L | Plan in `planning/vertex-ai-migration.md` (PR #67). **Phases 2–4 code complete** on `claude/magical-ptolemy-c6vogg` (draft PR, 2026-06-14): dual-mode client, WIF workflow, 6 tests, docs. **Phase 1 (GCP infra) = owner, still pending** — 3 secrets `WIF_PROVIDER`/`GCP_SA_EMAIL`/`GOOGLE_CLOUD_PROJECT`. **DO NOT MERGE the PR until Phase 1 + secrets done** (CI auth would fail). Then validate via `workflow_dispatch` → `ai_run_log.jsonl` shows `backend=vertex_ai`. |
