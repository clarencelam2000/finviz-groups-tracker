...
| WS5-8b | **Personal watchlist that rides the Morning scrape** (#319) | design pass first | L | 🔴 **Deferred to a dedicated session.** Think-big: force-include a user's watchlist ticker into the EOD picks scrape so it becomes a picks-adjacent row and rides Morning status. Two open questions (public-CSV privacy signal; TTL) + the "Watch vs I took it" unification with §8a. Full brief in `planning/trade-lifecycle-engine.md` §8b and #319. |
