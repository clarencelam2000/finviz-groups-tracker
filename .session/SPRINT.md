...
| WS5-5 | **Positions tab: recently-closed grace window** (#332) | `docs/index.html` | S | 🔴 Open. Follow-up from PR #331 (empty-state fix, `POS_VISIBLE_STATES` = open/managing/closing). `closed` positions still drop off the tab the instant they settle (owner `confirm-exit` or the engine's `autoConfirm`) — nothing lost server-side (`positions`/`position_events` keep full history in D1), purely a PWA display gap. Ask: show closed positions for a bounded window (e.g. N days, or last-N-closed) so the final fill/outcome is visible before it vanishes. Needs a design call on window length + card treatment (separate section vs. same list with a badge) — not yet scoped. **Not WS5-4** — that's push notifications + the confirm/still-holding action surface for `closing`-state positions; this is about already-settled trades. |
