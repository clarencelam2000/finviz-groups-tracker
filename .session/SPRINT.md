...
| ~~TAX-0~~ | ~~**Seed sector→industry taxonomy map**~~ | `scripts/seed_taxonomy.py`, `tests/test_seed_taxonomy.py`, `data/finviz_sector_industry_map.{json,csv}` | S | ✅ **Done 2026-06-24.** PR #171 merged. 11/11 sectors, 144/144 industries match (100%). 22 tests pass. |
| ~~TAX-1~~ | ~~**Ship industry→stock map artifact**~~ | `scripts/seed_taxonomy.py`, `tests/test_seed_taxonomy.py`, `data/finviz_industry_stock_map.json` | S | ✅ **Done 2026-06-24.** Same parse pass as TAX-0. 5550 stocks, ticker→industry reverse index, concentration scores. Unlocks: Worker ticker fallback (O(1), no FMP), peer context in Lookup tab, concentration risk signal, ETF override validation. 22 tests (9 new) pass. |
