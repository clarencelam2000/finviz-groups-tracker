...
| PICKS-3B | **3b: Risk engine + Focus List toggle (SPEC REFINED 2026-06-27)** | `docs/index.html`, `docs/sw.js`, `docs/releases.json`, `tests/test_pwa_picks.py` (NEW), `tests/fixtures/picks_latest.csv` | M | **3b.0 refactor first: extract `renderPickRow()`** (reused by 3c). Expandable risk panel (prev-day-high buy trigger, 20MA stop level + risk $/% via `Price×risk_20ma_pct`, 50MA wider alt, extension, + 3 tightness lines: Range/ATR, Volatility ATR%, Stop distance ATR). `All/Focus` toggle (resets to All on tab entry); Focus DQ `atr_ext_50>5` & `≤0`. Focus score = **one min–max ruler for all 3 components** (group `grp_sum_mid_rank` inv, **nearest-MA stop tightness** `min(positive risk_20,risk_50)` inv, `range_atr` inv) × **multiplicative** extension discount `(1−penalty)`, **always ∈[0,1]**; NOT 50MA proximity. Edge cases: all-equal→0.5, pool<5→rank-based, n=1→1.0. **Create Playwright test file from scratch** (no test_pwa_picks.py exists). Release triplet. See plan §3b. |
| PICKS-3C | **3c: Lookup Stage-2 section + deep-link button (SPEC REFINED 2026-06-27)** | `docs/index.html`, `docs/sw.js`, `docs/releases.json`, `tests/test_picks_button_config.py` (NEW) | S | Add Stage-2 section to **both `renderLookup()` branches** (group-by-name + ticker→group), reusing 3b's `renderPickRow()`. Deep-link button: **inline params + anti-drift test** vs `screener_config.json` `button` block (no runtime fetch), **inline JS slugify** (no CSV fetch), `ind_<slug>` for all 144 industries + **`sec_<slug>` for sectors (A3)** with 11-sector unit test. Release triplet. See plan §3c. |
