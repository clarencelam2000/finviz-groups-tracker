...
| RS-4 | **[FUTURE] Tier 5 RS discrete flags** | `scripts/delta_config.py`, `scripts/compute_deltas.py`, `docs/index.html` | L | Compute `beats_benchmark_X` (boolean per timeframe), `rs_new_high` (RS line at trailing peak), `rs_cross` (RS spread crossed 0 within N sessions) in the delta pipeline. Surface as card badges in vs-Market tab. Pipeline PR first, then UI PR. Deferred from Phase 3 — requires new compute columns not in Phase 2 scope. |
