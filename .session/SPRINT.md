...
| WS5-3b-ii | **Owner exit-transition routes + `autoConfirm` in the sweep** | `worker-positions/src/{index,sweep}.js`, `test/**` | M | 🔴 **Next.** Endpoints over the already-written pure functions in `advance.js`: `confirm-exit` (editable fill, pre-filled with `expected_exit_price`), `still-holding`, `correct-exit`, `reopen`. Then wire `autoConfirm()` into the sweep — needs a trading-session count between `exit_signal_date` and today; the distinct `trade_date`s in `ticker_quotes` are the natural session calendar. **Deliberately ordered after 3b-i:** shipping `autoConfirm` first would auto-close every exit at `EXIT_AUTOCONFIRM_SESSIONS` with no route for the owner to confirm or reject. Until this lands an exit parks in `closing` indefinitely (bars keep accruing — `closing` ∈ `HELD_STATES` — so no history is lost). |
| WS5-3b-OWNER | **Owner ratification: 2 wiring rules + the service-token widening** | — (decision only) | XS | 🔴 Open, none blocking. (1) Never advance on the entry-day bar — cost is a same-day collapse through the stop isn't seen until the next bar. (2) Stale bars report via the sweep's `stale` counter + CI log rather than the `position_events` ledger. (3) `POSITIONS_INGEST_TOKEN` may now trigger `/advance`, widening its documented blast radius; alternative is a third `POSITIONS_ENGINE_TOKEN` (one `wrangler secret put` + one GH secret — the seam already exists in `src/auth.js`). |
