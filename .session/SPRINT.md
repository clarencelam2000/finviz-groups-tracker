...
| PICKS-3 | **Phase 3: PWA surfaces** | `docs/index.html`, `docs/sw.js`, `docs/releases.json`, `knowledge/moaty-metrics.md` | L | **UNBLOCKED 2026-06-25** — live data flowing. Picks tab (reads `picks_latest.csv`, grouped by `list_category`→group→stocks; sort least-extended first; breadth count per group as health signal) + Lookup-tab Stage-2 section + deep-link button (tight `v=311` from `screener_config.json`). Release triplet + GUIDE entries for any new surfaced metric. See plan §PWA (D8) + plan §Documentation for cross-cutting doc requirements. |
