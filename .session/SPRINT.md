...
| AI-WATCH-1 | **[Watch] Phase classifier signal reduction — verify first live run** | — | XS | PR #94 rebuilt `build_phase_prompt` to use curated signal blocks instead of the full raw snapshot. Possible miss: Defensive phase (Utilities/Healthcare leading) only shows if those sectors appear in the sustained-strength top-N. After first `generate_ai.yml` run: check `sectors.rotation_phase.label`, confirm it matches the qualitative market picture, and verify the Defensive names appear in `serialize_strength_signals` output. If wrong 2 days running → add a compact sorted-by-YTD-rank line to `build_phase_prompt` (~30 tokens). Full rationale in `knowledge/decisions/adr-001-phase-classifier-signal-reduction.md`. |
