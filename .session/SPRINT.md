...
| PICKS-3B | **3b: Risk engine + Focus List toggle** | `docs/index.html`, `docs/sw.js`, `docs/releases.json`, `tests/` (Playwright fixture) | M | Per-row risk panel (prev-day-high trigger, 20MA stop default + risk $/% shown as %, 50MA wider alt, extension). `All/Focus` toggle inside Picks tab; Focus DQ `atr_ext_50>5` & below-50MA; blended quality rank (`grp_sum_mid_rank` inverted + `risk_20ma_pct` inverted + `range_atr` inverted, minus 3.5×→5× penalty capped at PENALTY_MAX=0.5; NOT 50MA proximity). Playwright fixture at `tests/fixtures/picks_latest.csv`. Release triplet. ANET risk worked example (price-based, EOD): risk_20ma_pct≈1.15%, risk_50ma_pct≈3.40% (formula: `(price−smaX_price)/price`, NOT High-based). |
...
| PICKS-3D | **3d: Polish (optional)** | `scripts/collect_picks.py`, `docs/index.html` | M | True inside-day (prev-day H/L self-join, schema bump) replacing the C1 proxy; loose fundamental floor on Focus + "Honorable mentions (failed fundamental floor)" sub-list (C9); search/filter, sort toggles, target/R framing. Also contains PICKS-3D-STALE (see below). |
