...
| `POWER_OF_3_ATR_MULT` | `2.0` | Backend metric (B-5, `picks_config.py`/`picks_metrics.py`): width, in ATR multiples, of the band that price, the 20MA and the 50MA must all fit inside for `power_of_3` to fire (1). Owner-specified band width, not an invented cutoff. Larger = looser (fires more often). To change, edit the constant — no selector_version bump needed (deterministic transform, not selection logic). |
