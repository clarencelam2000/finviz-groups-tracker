...
|--------|-------------------------------------|---------|
| `grp_rank_basis` | computed | `"sustained_strength"` (the 8) / `"freshness_fill"` (the 2) for leaders rows; **category name for the other buckets** - whoch rule won the slot, i.e `list_category` for non-leaders. Captures the within-leaders sub-rule for attribution — "did freshness fills underperform core slots?" |
