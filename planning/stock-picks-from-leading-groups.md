...
Add a pure helper module/function set (suggest `scripts/picks_metrics.py` or a section of
`picks_config.py`) computing, per row, from the already-stored Finviz columns. **All Finviz percent
columns are strings like `"51.84%"`; market cap like `"336.47B"` / `"850.00M"`; ATR/Price/High/Low
are plain floats.** Provide robust parsers (`_pct()`, `_cap_b()` handling `T/B/M/K`, empty→NaN).
...
- New columns appended to **both** `picks.csv` (migrate the one historical date, blank→recompute)
  and `picks_latest.csv`: `atr_ext_50, risk_20ma_pct, risk_50ma_pct, range_atr, stage2`.
...
PWA constant. (Verified: 273 → 165 rows / 158 unique tickers on 2026-06-25.)
...
|-----------|-----------|--------|-------|
| Group strength | **+** | `grp_*` (e.g. `grp_momentum_confirmed` desc, or inverse `grp_sum_mid_rank`) | "A++ from the strongest groups." Pick one canonical `grp_` metric; document it. |
| 20MA tightness | **+** | `risk_20ma_pct` (smaller = better) | CEO: "close to 20MA as positive weight" — tight logical stop = low risk. |
| Quiet bar | **+ (mild)** | `range_atr` (smaller = better) | C1 tightness proxy; constructive low-volatility day. |
| Extension penalty | **−** | `atr_ext_50` | **0 for `≤ ATR_EXT_PENALTY_START (3.5)`; linear ramp to `−PENALTY_MAX` from 3.5→5.0.** CEO explicit. |
...
| Focus weights | `w_group`, `w_tight20`, `w_quiet`, `PENALTY_MAX` (tune in spike) | Blended Focus quality score (§3b.2). |
...
- [ ] `atr_ext_50` matches the worked examples within rounding: ANET ≈ 0.96×, STX ≈ 3.2×,
      DELL ≈ 3.5×, SNDK ≈ 4.3× (2026-06-25 data).
- [ ] Picks tab renders, grouped category→industry→stock, base filter applied (≈165 rows on the
      2026-06-25 fixture), least-extended-first within each industry, breadth count per group.
...
- **PWA (Playwright fixture-intercept, per CLAUDE.md pattern):** route
  ...
  (`tests/fixtures/picks_latest.csv` — a small slice). Assert: tab appears; base filter row count;
  grouping + least-extended order; extension band colors; trim tag at ≥8×; `All/Focus` toggle
  filters out >5× and below-50MA names; Focus order respects the penalty; Lookup deep-link URL
  format; empty-CSV placeholder.
