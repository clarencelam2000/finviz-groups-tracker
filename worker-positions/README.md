...
| `POST` | `/watchlist` | Bearer | WS5 §8b P1: add `{ticker, level_type?, level_value?}` — UPSERT on `(user_id, ticker)`, so re-adding an existing ticker renews the TTL and updates the level. WS-POSITIONS-SEED: after the row lands, best-effort-seeds the ticker's newest completed daily bar into `ticker_quotes` (`source='fmp_seed'`, `src/seed.js`) so a brand-new ticker resolves to a real level on its next status read instead of waiting for the 17:30 held feed. The seed is fire-and-forget — never changes the response or fails the add. |
...
| `PATCH` | `/watchlist/<id>` | Bearer | WS5 §8b P1: `{renew:true}` resets the TTL/status; `{remove:true}` soft-removes (`status='removed'`, `removed_at` stamped — see morning-subtabs-watchlist below); `{restore:true}` undoes a remove back to `active` with a fresh TTL; or an edit-level body `{level_type?, level_value?}`. |
| `DELETE` | `/watchlist/<id>` | Bearer | WS5 §8b P1: hard-delete a watch entry. Only called on graduation (right after `POST /positions` succeeds) — the owner-facing "Remove" action in the PWA uses `PATCH {remove:true}` instead so the ticker survives in a collapsed "Recently removed" bin. |
| `GET` | `/watchlist-tickers` | Service token | WS5 §8b P1: for `scripts/collect_morning.py` — active watch tickers + `level_type` + latest-bar refs + `has_history:boolean` (WS-POSITIONS-STATUS, 2026-08-25 — true iff a `ticker_quotes` bar exists at all, lets the caller distinguish "never had data" from "Finviz missed it today"). **Omits `level_value`** (privacy — see `src/watchlist.js::watchlistTickerRefs`). |
...
| `WATCHLIST_PURGE_DAYS` | `14` | Calendar days an `expired` OR `removed` entry lingers (collapsed bin) before `tickWatchlist()` purges it (keyed off `expired_at`/`removed_at` respectively, not trading sessions). |
