...
| `JOB_SCHEDULE[*].hour` / `.minute` | `collect_morning` 09:45, `collect_preclose` 15:50, `collect_eod` 17:00, `picks` 17:00 (ET, gated — see § Picks dependency gate) | Per-job target wall-clock time. |
...
|-----|-----------------------|---------|
| `collect_morning` | 09:45 (ungated) | WS3 morning status — tags prior-session picks with a Triggered/Setting-up/Gapped-through/Failed-breakout/Invalidated/No-quote status (ADR-013); dispatches `collect_morning.yml`; KV key `last_dispatch_collect_morning` |
